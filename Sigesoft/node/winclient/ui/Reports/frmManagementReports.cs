...
namespace Sigesoft.Node.WinClient.UI.Reports
{
    public partial class frmManagementReports : Form
    {
        ...
        private void GeneratePDF(string componentId)
        {
            switch (componentId)
            {
                ...
                case Constants.INFORME_MEDICO_OCUPACIONAL_COSAPI:
                    ...
                    _filesNameToMerge.Add(string.Format("{0}.pdf", Path.Combine(ruta, _serviceId + "-" + componentId)));
                    break;
                case Constants.CERTIFICADO_APTITUD_MEDICO_COSAPI:
                    GenerateCertificadoAptitudMedicoOcupacional_Cosapi(string.Format("{0}.pdf", Path.Combine(ruta, _serviceId + "-" + Constants.CERTIFICADO_APTITUD_MEDICO_COSAPI)));
                    _filesNameToMerge.Add(string.Format("{0}.pdf", Path.Combine(ruta, _serviceId + "-" + componentId)));
                    break;
        ...
        private void ChooseReport(string componentId, string serviceId, string pPacienteId, int pintIdCrystal)
        {
            ...
            rp = new Reports.crEvaluacionCardiologicaSM();                     
                ...
                case Constants.INFORME_FICHA_MEDICA_TRABAJADOR_CI:
                    ...
                    _filesNameToMerge.Add(string.Format("{0}.pdf", Path.Combine(ruta, _serviceId + "-" + componentId)));
                    break;
                case Constants.ESPIRO_PRUEBA:
                    GeneratePruebaEspiro(string.Format("{0}.pdf", Path.Combine(ruta, _serviceId + "-" + Constants.ESPIRO_PRUEBA)), _serviceId, _pacientId);
                    //GenerateInformeMedicoTrabajadorInternacional(string.Format("{0}.pdf", Path.Combine(ruta, _serviceId + "-" + Constants.INFORME_FICHA_MEDICA_TRABAJADOR_CI)), _serviceId, _pacientId);
                    _filesNameToMerge.Add(string.Format("{0}.pdf", Path.Combine(ruta, _serviceId + "-" + componentId)));
                    break;  
                default:
        ...
        private void GeneratePruebaEspiro(string pathFile, string ServicioId, string PacienteId)
        {
            var ValoresEspiro = _serviceBL.ValoresComponente(_serviceId, Constants.ESPIROMETRIA_ID);

            CreateEspiroPrueba.CreateInformeEspiroPrueba(pathFile, ValoresEspiro);
